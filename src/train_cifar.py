...
from callbacks.diff import DiffOutputCallback, DiffHiddenCallback
from data.wrappers import TorchvisionDatasetWrapper as Wrp
from runners.kd import KDRunner
from utils.model_loading import load_model_from_path
from utils.loss_coefs import get_loss_coefs
...
def main(args):
    ...
    train_dataset = Wrp(datasets.CIFAR10(
        root=os.getcwd(), train=True, transform=transform_train, download=True
    ))
    valid_dataset = Wrp(datasets.CIFAR10(
    ...
    if is_kd:
        ...
        aggregator_callback = dl.MetricAggregationCallback(
            prefix="loss", metrics=metrics, mode="weighted_sum"
        )
        wrapped_agg_callback = dl.ControlFlowCallback(aggregator_callback, loaders=["train"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="weight for output diff loss",
    )
    parser.add_argument(
        "--beta",
        default=None,
        type=float,
        help="weight for hidden diff loss"
    )
